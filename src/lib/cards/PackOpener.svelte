<script lang="ts">
  ...
  onMount(() => {
    // Pack is larger than the cards in both dimensions so they fit inside with a
    // sealed margin all round (cards are 2.7 x 3.78 below).
    const PACK_W = 3.5,
      PACK_H = 5.0;
    const TEAR_BASE = 0.93; // tear line height (thin sealed strip above the cards)
    ...
    const CARD_W = 2.7,
      CARD_H = CARD_W * (7 / 5),
      CARD_D = 0.04; // very thin card slab

...
// The whole gamba page dataset, built for /api/gamba. The page itself has NO
// server load: its universal load fires the fetch without awaiting, so navigating
// to /gamba completes instantly and this streams in behind the skeleton.
export async function buildGambaData(user: SessionUser) {
	...
	const [vp_balance, gold_balance, wallet, packsRes, cardsRes, ownedRes, raresRes, lastLootDate, lootConfig, crateDropsRes, ownedPacksRes] = await Promise.all([
		...
		db().from('vs_user_cards').select('card_id').eq('user_id', user.id),
		...
		db()
			...
			.limit(20),
		getLastLootDate(user.discord_id, user.rsn),
		getLootConfig(),

<script lang="ts">
	...
	// The full bingo board page body, rendered by /events/[slug] when the event's
	// kind is bingo (one URL per event slug, whatever its kind). The parent owns
	// the streamed payload / skeleton / not-found handling and passes the resolved
	// payload down; type-only server imports are erased at build time.
	type BingoOk = Extract<BingoDetail, { kind: 'ok' }>;
	...
	let {
		...
		detail: Omit<BingoOk, 'kind'>;
		user: SessionUser | null;
		isAdmin: boolean;
	} = $props();

	const data = $derived({ user, isAdmin, ...detail });

...
const MAX_ENTRIES = 1500; // ~600 tiles plus hovercards and rank icons, with room
...
async function fetchOne(url: string): Promise<Hit | null> {
	...
	if (new URL(res.url).hostname !== WIKI_HOST) return null;
	...
	if (!job) {
		job = (async () => {
			...
			for (const base of bases) {
				...
				const hit = await fetchOne(withWidth);
				if (hit) return hit;
			}
			return null;

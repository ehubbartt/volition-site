<script lang="ts">
  ...
  let lastAction = $state<null | { kind: "approve" | "reject"; rsn: string }>(
    null,
  );
  ...
  <article class="card">
    <header class="card-head">
      <div class="who">
        ...
        <div class="who-text">
          <strong class="rsn"
            >{current.user.rsn ?? current.user.discord_username}</strong
          >
      ...
      <div class="progress muted">
        {currentIndex + 1} of {data.groups.length}
      </div>
    </header>
    ...
    <section class="tile-block">
      <div class="tile-line">
        <span
          class="tier-pill"
          style="background: {tileTier.color}; color: #1a1208"
        >
          {tileTier.label}
        </span>
        <span class="tile-row muted">Row {current.tile.row}</span>
        <span class="tile-pts"
          >{tilePoints} pt{tilePoints === 1 ? "" : "s"}</span
        >
      </div>
    ...
    <section class="proofs">
      <h3>
        {current.submissions.length} proof{current.submissions.length === 1
          ? ""
          : "s"}
      </h3>
      <ul class="proof-list">
    ...
    {#if lastAction}
      <p class="last-action muted">
        Last: {lastAction.kind === "approve" ? "approved" : "rejected"}

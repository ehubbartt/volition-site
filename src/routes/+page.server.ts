...
export const load: PageServerLoad = async ({ parent }) => {
	...
	if (!user) {
		const [calendar, playersRes, eventsRes] = await Promise.all([
			...
			sb.from('vs_events').select('status, ends_at').is('owner_user_id', null).in('status', ['open', 'locked', 'closed'])
		]);
	...
	const [calendar, playersRes, siteUsersRes, eventsRes, packsRes, tasks] = await Promise.all([
		...
		sb.from('vs_events').select('status, ends_at').is('owner_user_id', null).in('status', ['draft', 'preview', 'open', 'locked', 'closed']),

...
`players` table = Volition), and the "Who's in" panel splits into **Volition** and the
opposing camp (**IronClad** — `OPPONENT_LABEL` in `src/lib/clans.ts`) whenever both camps
have signed up; a members-only signup stays one flat list.

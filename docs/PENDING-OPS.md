...
| `db/scripts/rank_manual_adjustments.sql` | ✅ staging · **PENDING prod** | Manual rank adjustments: the `vs_rank_overrides` table + `source`/`quantity` on `vs_rank_item_claims`. Apply BEFORE deploying the build with /admin/ranks/adjustments — the page 500s on the missing table, and every rank check selects the two new claim columns. Depends on `rank_item_claims.sql` being applied first. Safe to re-run. Check: `select 1 from vs_rank_overrides limit 1;` |
